...
void main() {
  group('TransformedSchema flag inheritance', () {
    test('transform should inherit isOptional flag', () {
      ...
      print('Wrapped schema isOptional: ${(schema as dynamic).schema.isOptional}');
      print('TransformedSchema isOptional: ${schema.isOptional}');

      expect(schema.isOptional, isTrue,
        reason: 'TransformedSchema should inherit isOptional from wrapped schema');
    });

    test('transform should inherit isNullable flag', () {
      final schema = Ack.string().nullable().transform((val) => val ?? '');

      print('Wrapped schema isNullable: ${(schema as dynamic).schema.isNullable}');
      print('TransformedSchema isNullable: ${schema.isNullable}');

      expect(schema.isNullable, isTrue,
        reason: 'TransformedSchema should inherit isNullable from wrapped schema');
    });
    ...
    test('optional+nullable+transform in object context should work', () {
      final schema = Ack.object({
        'name': Ack.string(),
        'nickname': Ack.string().optional().nullable().transform((val) => val ?? 'no-nick'),
      });
      ...
      expect(result1.isOk, isTrue, reason: 'Should accept missing optional field');
      expect(result2.isOk, isTrue, reason: 'Should accept null for nullable field');
