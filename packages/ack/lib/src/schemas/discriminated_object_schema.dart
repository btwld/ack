...
final class DiscriminatedObjectSchema<T extends Object> extends AckSchema<T>
  ...
  SchemaResult<Object> parseAndValidateRepresentation(
    ...
    final hasDefaultInput = inputValue == null && defaultValue != null;
    final sourceValue = hasDefaultInput
    ...
    if (sourceValue == null) {
      ...
      final nullResult = failNonNullable(context);
      return switch (nullResult) {
        Ok(value: final value) => SchemaResult.ok(value),
        Fail(error: final error) => SchemaResult.fail(error),
      };
    }

    if (inputValue == null && defaultValue != null && sourceValue is! Map) {
      return SchemaResult.fail(
        SchemaValidationError(
          message:
