...
void main() {
  ...
  print('  null (non-nullable): ${nullResult.isFail ? "REJECTED" : "ACCEPTED"}');
  ...
  print('  null input with default: ${withDefault.validate(null).getOrThrow()}');
  ...
  print('  "hello" (length > 3): ${refinedAny.validate("hello").isOk ? "PASS" : "FAIL"}');
  print('  "hi" (length <= 3): ${refinedAny.validate("hi").isOk ? "PASS" : "FAIL"}');
  print('  12345 (length > 3): ${refinedAny.validate(12345).isOk ? "PASS" : "FAIL"}');
  print('  42 (length <= 3): ${refinedAny.validate(42).isOk ? "PASS" : "FAIL"}');
  ...
  print('  Config with object settings: ${configSchema.validate(config1).isOk ? "VALID" : "INVALID"}');
  print('  Config with array settings: ${configSchema.validate(config2).isOk ? "VALID" : "INVALID"}');
  ...
  print('  API call validation: ${apiSchema.validate(apiCall).isOk ? "VALID" : "INVALID"}');
  ...
  print('  Legacy data validation: ${legacySchema.validate(legacyData).isOk ? "VALID" : "INVALID"}');
}
