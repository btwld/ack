...
4.  **Validate Data:**
    Use the `validate()` method of your schema to check the data. This returns a `Result` object.
...
5.  **Handle the Result:**
    Check the `isOk` property of the `Result`. If `true`, validation passed. If `false`, validation failed.
