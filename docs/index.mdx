...
- **Validate external payloads**: Enforce required fields, type constraints,
  and schema rules at input/output boundaries.
...
- Works directly with JSON data from APIs
...
Ack includes these core capabilities for data validation and transformation:
...
- **[JSON Serialization](/core-concepts/json-serialization)**: Validate JSON payloads, work with validated data, and add typed wrappers when needed
...
Ack supports these advanced patterns for complex validation scenarios:
...
- **[Codecs and transformations](/core-concepts/schemas#built-in-codecs)**: Convert between boundary and runtime values with built-in codecs or `Ack.codec(...)`; use `.transform()` for one-way parse transformations
