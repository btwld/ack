...
- `Ack.list(AckSchema<Boundary, Runtime> itemSchema)`: Creates a `ListSchema` for validating arrays.
- `Ack.object(Map<String, AnyAckSchema> properties)`: Creates an `ObjectSchema` for validating objects.
- `Ack.enumValues(List<T> values)`: Creates an `EnumSchema<T>` for Dart enum types. Parses enum names from string boundary values and encodes runtime enum values back to their names. **Preferred over `enumString` when a Dart enum exists.**
...
- `Ack.anyOf(List<AnyAckSchema> schemas)`: Creates an `AnyOfSchema` for union types.
- `Ack.any()`: Creates an `AnySchema` that accepts any non-null JSON-safe value.
- `Ack.discriminated<T extends Object>({required String discriminatorKey, required Map<String, AckSchema<JsonMap, T>> schemas})`: Creates a discriminated union schema. Branches may be plain `ObjectSchema` or codec schemas whose base is an `ObjectSchema`.
...
- `SchemaResult<Runtime> safeParse(Object? data, {String? debugName})`: Validates the input data and returns a result. Never throws exceptions - returns `SchemaResult` with either success or failure.
...
- `SchemaResult<Boundary> safeEncode(Runtime? value, {String? debugName})`: Encodes a runtime value to its boundary representation and returns a result instead of throwing.
- `Boundary? encode(Runtime? value, {String? debugName})`: Encodes a runtime value to its boundary representation. Throws `AckException` if encoding fails.
- `SchemaResult<Runtime> validate(Object? data, {String? debugName})`: Deprecated alias for `safeParse`.
- `Runtime? tryParse(Object? data, {String? debugName})`: Deprecated helper that returns `safeParse(...).getOrNull()`.

### Schema Modification Methods

- `AckSchema<Boundary, Runtime> nullable()`: Returns a new schema that also accepts `null` values.
...
- `AckSchema<Boundary, Runtime> withDefault(Runtime value)`: Sets a default value when input is `null`.
...
- `AckSchema<Boundary, Runtime> withConstraint(Constraint<Runtime> constraint)`: Applies a custom validation constraint.
...
- `CodecSchema<Boundary, R> transform<R>(R Function(Runtime) transformer)`: Creates a parse-only codec that transforms validated values to a different type.
...
Schema for validating booleans. Validates `true` and `false` values.
...
- Constructed using `Ack.object(Map<String, AnyAckSchema> properties, {bool additionalProperties = false})`.
...
- Use `.extend(Map<String, AnyAckSchema> newProperties)` to add more properties.
...
- Parses enum names from string boundary values and returns typed enum values.
- Encodes runtime enum values back to their `.name` strings.

### `AnyOfSchema`

Schema for union types (value must match one of several schemas).

- Created using `Ack.anyOf(List<AnyAckSchema> schemas)`

### `DiscriminatedObjectSchema`

Schema for polymorphic validation based on a discriminator field.

- Created using `Ack.discriminated<T extends Object>({required String discriminatorKey, required Map<String, AckSchema<JsonMap, T>> schemas})`

### `AnySchema`

Schema that accepts any non-null JSON-safe value without shape validation.

- Created using `Ack.any()`
- Use `.nullable()` if `null` should also be accepted.
- Useful for gradual migration or maximum flexibility

### `CodecSchema<Boundary, Runtime>`

Schema that maps between boundary values and runtime values.

- Created using `schema.transform<R>(R Function(Runtime) transformer)` for parse-only transforms.
- Created using `schema.codec<R>(decode: ..., encode: ...)` or built-ins like `Ack.date()` for bidirectional codecs.
