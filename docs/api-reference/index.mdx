...
- `OptionalSchema<T> optional()`: Makes the schema optional (can be omitted from objects).
...
Object returned by `validate()`. See [Error Handling](../core-concepts/error-handling.mdx).
...
Schema wrapper that makes a field optional in object validation.

- Created using `schema.optional()`
