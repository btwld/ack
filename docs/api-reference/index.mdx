...
- `Ack.instance<T>()`: Creates an `InstanceSchema<T>` for validating runtime objects by type.
- `Ack.codec<I, O>(AckSchema<I> inputSchema, AckSchema<O> outputSchema, {required O Function(I) decode, required I Function(O) encode})`: Creates a bidirectional codec between boundary and runtime types.
- `Ack.discriminated<T extends Object>({required String discriminatorKey, required Map<String, AckSchema<T>> schemas})`: Creates a discriminated union schema. Branches may be plain `ObjectSchema` or codec schemas whose input side is an `ObjectSchema`.
...
- `CodecSchema<T, R> transform<R extends Object>(R Function(T) transformer)`: Creates a one-way codec that transforms validated non-null values to a different type.
...
Schema that converts between boundary values and runtime values.

- Created as a one-way codec using `schema.transform<R>(R Function(T) transformer)`
- Created as a bidirectional codec using `Ack.codec<I, O>(...)`
