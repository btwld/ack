...
- `Ack.fromJsonSchema(Object document, {Uri? baseUri, Map<Uri, Object>
  documents = const {}})`: Strictly imports a decoded draft 2020-12 JSON
  Schema. Referenced documents must be supplied explicitly; Ack never fetches
  files or URLs.
...
- `importJsonSchema(Object document, {Uri? baseUri, Map<Uri, Object> documents
  = const {}, bool allowUnsupported = false})`: Imports a decoded schema and
  returns a `JsonSchemaImportResult`. The default is strict. With
  `allowUnsupported: true`, unsupported assertions are omitted and reported;
  malformed supported keywords, unresolved references, invalid dialects, and
  unsafe reference cycles still throw.
- `JsonSchemaImportResult`: Contains the executable
  `AckSchema<Object, Object> schema`, immutable `diagnostics`, and `isExact`,
  which is true when no reachable assertion was omitted.
- `JsonSchemaImportDiagnostic`: Describes one import issue through `code`,
  `documentUri`, `pointer`, `keyword`, and `message`.
- `JsonSchemaImportException`: Thrown when a strict import encounters an
  unsupported assertion or when an import cannot be completed safely. Its
