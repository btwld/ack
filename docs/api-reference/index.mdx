...
- `Map<String, Object?> toJsonSchema()`: Renders the schema through the canonical `AckSchemaModel` JSON Schema output.
...
- Render to JSON Schema with `schema.toSchemaModel().toJsonSchema()`, the same output returned by `schema.toJsonSchema()`
