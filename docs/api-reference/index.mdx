...
- `TOut parseAs<TOut>(Object? data, TOut Function(T? validated) map, {String? debugName})`: Validates, maps, and throws on validation or mapper failures.
- `SchemaResult<TOut> safeParseAs<TOut>(Object? data, TOut Function(T? validated) map, {String? debugName})`: Validates, maps, and returns `SchemaResult` without throwing.
...
- Use `Ack.literal(String value)` for exact string matching.
- Use `Ack.enumString(List<String> values)` for ad-hoc string enums.
