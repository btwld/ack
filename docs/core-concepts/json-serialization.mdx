...
### Generating Extension Types from Standalone Schemas

When you define schemas manually (without a backing class), annotate the
variable or getter with `@AckType()` to have the generator build an extension
type that wraps the validated value.
...
Running `dart run build_runner build` produces two things inside
`user_schema.g.dart`:

- the original `userSchema` definition (so you can keep importing it from one
  place), and
- a `UserType` extension type with `parse`/`safeParse`, typed getters, and
  `toJson()` helpers (removes "Schema" suffix, adds "Type").
...
Any nested schema annotated with `@AckType()` generates its own extension type,
but nested object fields return `Map<String, Object?>` rather than the typed
extension type.
