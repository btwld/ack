...
If you use Ack's code generation, the generated schema class provides `toModel()` methods for seamless conversion.
...
// Inside the validation success block:
if (result.isOk) {
  // Create schema instance with the validated data
  final userSchemaInstance = UserSchema(result.getOrThrow());
  // Convert the schema instance to the User model instance
  if (userSchemaInstance.isValid) {
    final userModelInstance = userSchemaInstance.toModel();
    print('Created User model via generator: ${userModelInstance.name}');
  }
}
```
