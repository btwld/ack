...
There are two ways to use the generated schema class:
...
#### Using fromModel for existing models
...
Converts the schema to an OpenAPI definition as a Map.

```dart
final definition = UserSchema.toDefinition();
print('OpenAPI Definition: $definition');
```

#### `static String toDefinitionString()`

Converts the schema to an OpenAPI definition as a formatted JSON string.

```dart
final definitionString = UserSchema.toDefinitionString();
print('OpenAPI Definition JSON:\n$definitionString');
```
