...
There are two main ways to use the generated schema class:

#### Approach 1: Using the constructor (instance-based validation)
...
#### Approach 2: Converting existing models to schema data
...
void main() {
  ...
  // Convert model to map data for schema validation using toMap()
...
```dart
// Using constructor + toModel
final schema = UserSchema(userData);
if (schema.isValid) {
  final user = schema.toModel();
  print('User: ${user.name}');
} else {
  print('Validation failed: ${schema.getErrors()}');
}
```
