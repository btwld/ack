...
For converting validated data to your model, use the instance-based approach:
...
if (userSchema.isValid) {
  // Convert to your fully typed model
  final User user = userSchema.toModel();
  ...
  final error = userSchema.getErrors();
  print('Invalid data: ${error?.message}');
}
```
