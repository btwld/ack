...
Primitive schemas do not coerce values. They validate the runtime type they
represent: `Ack.string()` accepts strings, `Ack.integer()` accepts integers,
`Ack.double()` accepts doubles, and `Ack.boolean()` accepts booleans.

Use `Ack.codec(...)` when a boundary format needs conversion before it becomes
your runtime type.

```dart
final stringToIntCodec = Ack.codec<String, int>(
...
stringToIntCodec.safeParse('42');  // ✅ OK: parses to 42
stringToIntCodec.safeParse(42);    // ❌ FAIL: input must be a string
Ack.integer().safeParse('42');  // ❌ FAIL: no implicit coercion
Ack.integer().safeParse(42);    // ✅ OK
```

Use codecs when you need to:

- **Validate API requests** where wire formats differ from runtime types
- **Distinguish between types** in union schemas (for example string `"123"` vs integer `123`)
- **Normalize boundary values** while keeping primitive schemas strict
- **Preserve round-trips** with `parse(...)` and `encode(...)`
