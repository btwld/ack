...
### `date()`
Requires a valid date string in `YYYY-MM-DD` format. Returns `String`. For a
`DateTime` runtime value, use the [`Ack.date()`](../core-concepts/schemas#date-and-time-codecs)
typed codec instead.
```dart
Ack.string().date()
```
...
### `datetime()`
Requires a valid ISO 8601 date-time string. Returns `String`. For a `DateTime`
runtime value, use the [`Ack.datetime()`](../core-concepts/schemas#date-and-time-codecs)
typed codec instead.
```dart
Ack.string().datetime()
```
...
Ack primitive schemas validate exact Dart boundary types. They do not coerce
between strings, numbers, and booleans.

```dart
Ack.string().safeParse('hello'); // ✅ OK
Ack.string().safeParse(123);     // ❌ FAIL

Ack.integer().safeParse(42);     // ✅ OK
Ack.integer().safeParse('42');   // ❌ FAIL
Ack.integer().safeParse(42.0);   // ❌ FAIL

Ack.double().safeParse(3.14);    // ✅ OK
Ack.double().safeParse(42);      // ❌ FAIL
Ack.number().safeParse(42);      // ✅ OK: accepts int or double
```

Use `.codec(...)` when boundary data needs an explicit conversion step:

```dart
final numericString = Ack.string().codec<int>(
  decode: int.parse,
  encode: (value) => value.toString(),
);

numericString.safeParse('42'); // ✅ OK: 42
numericString.safeParse(42);   // ❌ FAIL: boundary must be a string
```
