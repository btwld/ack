...
### `minLength(int min)`
Minimum length check: value must contain at least `min` characters.
```dart
Ack.string().minLength(5)
```

### `maxLength(int max)`
Maximum length check: value must contain at most `max` characters.
```dart
...
### `notEmpty()`
Shortcut for non-empty strings (`minLength(1)`).
```dart
Ack.string().notEmpty()
```

### `matches(String pattern, {String? example, String? message})`
Applies a regular expression match against `pattern`.
...
Checks that `pattern` appears somewhere in the string.
```dart
...
### `uri()`
Validates URI syntax against RFC 3986.
```dart
Ack.string().uri()
```
...
Restricts values to the provided `allowedValues` list.
```dart
...
### `url()`
Alias for `uri()`. Validates URL syntax.
```dart
Ack.string().url()
```

### `ip({int? version})`
Validates IP addresses. Set `version` to restrict to IPv4 or IPv6.
```dart
...
### `min(num limit)`
Lower bound check (inclusive): value must be `>= limit`.
```dart
Ack.integer().min(0) // >= 0
...
### `max(num limit)`
Upper bound check (inclusive): value must be `<= limit`.
```dart
...
Exclusive lower bound: value must be `> limit`.
```dart
...
### `lessThan(num limit)`
Exclusive upper bound: value must be `< limit`.
```dart
...
Requires the number to be a multiple of `factor`.
```dart
...
### `positive()`
Accepts only values greater than 0.
```dart
...
### `negative()`
Accepts only values less than 0.
```dart
...
### `safe()` (Integer only)
Limits integers to JavaScript's safe integer range (`-2^53+1` to `2^53-1`).
```dart
Ack.integer().safe()
```
...
### `minLength(int min)`
Minimum item count: list must contain at least `min` entries.
```dart
Ack.list(Ack.string()).minLength(1)
```

### `maxLength(int max)`
Maximum item count: list must contain at most `max` entries.
```dart
Ack.list(Ack.integer()).maxLength(10)
```

### `length(int count)`
Exact item count: list must contain exactly `count` entries.
```dart
Ack.list(Ack.boolean()).length(5)
```

### `notEmpty()`
Shortcut for non-empty lists (`minLength(1)`).
```dart
Ack.list(Ack.object({})).notEmpty()
```

### `unique()`
Requires all items to be unique. Uses deep structural equality, so nested
maps/lists are compared by value.
```dart
