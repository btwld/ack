...
### `pattern(String pattern, {String? patternName, String? example})`
Ensures the string matches the given regular expression `pattern`.
```dart
// Simple alphanumeric pattern
Ack.string.pattern(r'^[a-zA-Z0-9]+$')

// UUID pattern
Ack.string.pattern(r'^[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}$')
```
...
final usernameSchema = Ack.string
  ...
  .pattern(r'^[a-z0-9_]+$') // Then check pattern (lowercase alphanumeric/underscore)
