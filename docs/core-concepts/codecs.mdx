...
An `Error` thrown by an `encode` function is a programming bug, not bad data, so
it propagates out of `safeEncode` with its original stack trace at every nesting
level — inside an object, list, map, or union, too. Throw an `Exception` (or
express the rule with an `output:` schema) to signal a value the codec cannot
encode.
...
Constraints added to a codec validate its **runtime** values, which the encoder
may map to something else entirely, so they are enforced locally but not
published: the export omits the keyword and `toSchemaModel().warnings` reports a
`codec_runtime_constraint_not_exported` warning. Put rules the wire should
advertise on the input schema instead — `Ack.string().minLength(5).codec(...)`
exports `minLength: 5`, while `Ack.string().codec(...).minLength(5)` does not.
`Ack.duration()` is the exception: its boundary is the milliseconds its `min()`
and `max()` already compare, so those constraints do export.
