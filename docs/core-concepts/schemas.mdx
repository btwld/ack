...
final usernameSchema = Ack.string
  ...
  .constrain(
    StringRegexConstraint(
      ...
      pattern: r'^[a-zA-Z0-9_]+$', // Regex: letters, numbers, underscore
      example: 'user_123'
    )
  );
...
final userSchema = Ack.object({
  ...
  'address': Ack.object({
    'street': Ack.string,
    'city': Ack.string,
    'zipCode': Ack.string.pattern(r'^\d{5}$') // Simple 5-digit zip code regex
...
final optionalAddressSchema = Ack.object({
  'street': Ack.string,
  'city': Ack.string,
}, required: ['street', 'city']).nullable(); // Note .nullable() is outside Ack.object
