...
```dart
// Basic string — strict by definition; only accepts String.
...
// Date/datetime string format validators — parse returns String.
final dateStringSchema = Ack.string().date(); // YYYY-MM-DD
final datetimeStringSchema = Ack.string().datetime(); // ISO 8601
...
Every schema can parse boundary input and encode runtime values back to
boundary form. Most schemas use the same type on both sides, such as
`String` for `Ack.string()`. When the types differ, Ack uses
`CodecSchema<I, O>` where `I` is the boundary type and `O` is the
runtime type.

```dart
final rawDateString = Ack.string().date();
rawDateString.parse('2026-05-10'); // String

final dateObject = Ack.date();
dateObject.parse('2026-05-10'); // DateTime(2026, 5, 10)
dateObject.encode(DateTime(2026, 5, 10)); // '2026-05-10'

final homepage = Ack.uri();
homepage.parse('https://example.com'); // Uri
```

`Ack.date()`, `Ack.datetime()`, `Ack.uri()`, and `Ack.duration()` are
built-in codec schemas. They validate their JSON-friendly boundary
shape, decode into Dart runtime objects, and encode those runtime
objects back to the boundary shape.

### Number

Ack ships three numeric factories. `Ack.integer()` and `Ack.double()`
are strict per Dart subtype — `Ack.integer().parse(1.0)` fails, and
`Ack.double().parse(1)` fails. `Ack.number()` mirrors JSON Schema's
`"type": "number"` union and accepts any `num` (both `int` and
`double`). Reach for `Ack.number()` when a field is allowed to be
either subtype; use the strict variants when only one is valid; use
`Ack.codec(...)` when you need to convert across types (e.g. `String`
→ `int`).

```dart
// Integer validation — strict, rejects 1.0
final ageSchema = Ack.integer()
  .min(0)
  .max(120);

// Double validation — strict, rejects 1
final priceSchema = Ack.double()
  .positive()
  .multipleOf(0.5); // Use factors that avoid floating point rounding issues

// Number validation — accepts both int and double
final temperatureSchema = Ack.number().min(-273.15);
temperatureSchema.parse(20);    // ok — int
temperatureSchema.parse(20.5);  // ok — double
...
```dart
// String or integer — primitives are strict by default.
...
Transform validated data on the parse path. `schema.transform(...)`
returns a one-way codec with no encoder, so encoding through that
schema fails with `SchemaEncodeError.oneWayTransform`. Use
`Ack.codec(...)` when the conversion must be bidirectional.
...
final dateSchema = Ack.string()
  ...
  .transform<DateTime>(DateTime.parse);
