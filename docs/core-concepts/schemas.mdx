...
// Date/datetime strings (returns String — pattern-validated only)
final dateString = Ack.string().date(); // YYYY-MM-DD
final datetimeString = Ack.string().datetime(); // ISO 8601
...
For schemas whose runtime value is a Dart `DateTime`, `Uri`, or `Duration`
(string boundary in, typed runtime out), use the typed codecs documented
below in [Date and time codecs](#date-and-time-codecs).
...
Accept any non-null JSON-safe value without shape validation (use sparingly):
...
For schemas whose runtime value is a Dart `DateTime`, `Uri`, or `Duration`, use
the bidirectional codecs. They parse a JSON-safe string boundary into the typed
runtime value and encode it back.

```dart
// AckSchema<String, DateTime> — boundary: 'YYYY-MM-DD', runtime: DateTime
final dateSchema = Ack.date();

// AckSchema<String, DateTime> — boundary: ISO 8601 datetime, runtime: DateTime
final datetimeSchema = Ack.datetime();

// AckSchema<String, Uri> — boundary: URI string, runtime: Uri
final uriSchema = Ack.uri();

// AckSchema<String, Duration> — boundary: ISO 8601 duration, runtime: Duration
final durationSchema = Ack.duration();
```

`.min()` / `.max()` constraints on `Ack.date()` require a local-midnight
`DateTime` reference; on `Ack.datetime()` they require a UTC reference. The
exported JSON Schema bound is shaped to match (`YYYY-MM-DD` for date, full ISO
8601 for date-time).
...
On encode, optional non-nullable properties whose runtime value is `null` are
omitted from the output. This lets model encoders write fields directly, such
as `'age': value.age`, while still preserving parse-side strictness for
explicit `null` input.
