...
void main() {
  ...
  // 5. Convert model to schema (using toMap() method)
...
### `matches()` Method for Full Pattern Validation

The `matches()` method validates that a string fully matches a pattern (automatically anchored with ^ and $).
...
### `contains()` Method for Partial Pattern Matching

The `contains()` method validates that a string contains a pattern anywhere within it.

```dart
// Password must contain at least one uppercase letter
final passwordSchema = Ack.string.contains(r'[A-Z]');

print(passwordSchema.validate('Password123').isOk); // true - contains uppercase
print(passwordSchema.validate('password123').isOk); // false - no uppercase
```
