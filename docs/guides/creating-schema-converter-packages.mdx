...
final JsonSchema jsonSchema = schema.toJsonSchema();
```
...
`JsonSchema` is ACK's concrete, zero-cost JSON Schema value type. It remains
usable as a `Map<String, Object?>`, so JSON-map targets should call
`schema.toJsonSchema()` directly. Imported schemas use the existing
`AckAllOfSchemaModel` and `AckRefSchemaModel` variants, with their preserved
definition bodies attached to the root model. They do not introduce an
import-only variant into the sealed model hierarchy.
