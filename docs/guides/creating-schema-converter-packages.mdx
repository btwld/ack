...
Imported schemas use the existing `AckAllOfSchemaModel` and
`AckRefSchemaModel` variants, with their preserved definition bodies attached
to the root model. They do not introduce an import-only variant into the sealed
model hierarchy.
