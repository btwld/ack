...
Imported JSON Schema values surface as `AckImportedSchemaModel`. That variant
preserves original keyword applicability instead of inferring a `type`. JSON-map
targets should emit `schema.toJsonSchema()`. Targets without a generic JSON
Schema fragment should reject the case explicitly, the same way they handle
unsupported `AckRefSchemaModel` recursion.
...
class <Target>SchemaConverter {
  ...
  static <TargetSchema> _convertModel(AckSchemaModel schema) {
    return switch (schema) {
      ...
      // Imported fragments keep original JSON Schema keywords. JSON-map
      // targets should return schema.toJsonSchema(); typed builder APIs
      // that cannot represent an untyped keyword bag should reject.
      AckImportedSchemaModel() => throw UnsupportedError(
...
TargetSchema _convert(AckSchemaModel schema) {
  return switch (schema) {
    ...
    // Emit the generic Draft-7 map when the target can consume JSON Schema.
    // If it cannot represent an untyped fragment, throw UnsupportedError.
    AckImportedSchemaModel() => TargetSchema.jsonSchema(schema.toJsonSchema()),
  };
}
