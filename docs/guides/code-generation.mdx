...
The generated schema classes include methods to convert your schema to OpenAPI definition format, which is useful for API documentation or integration with OpenAPI tools.

```dart
// Get the OpenAPI definition as a Map
final definition = UserSchema.toDefinition();

// Get the OpenAPI definition as a formatted JSON string
final definitionString = UserSchema.toDefinitionString();
print('OpenAPI Definition:\n$definitionString');
