...
Ack [schemas](../core-concepts/schemas.mdx) can be converted into JSON Schema objects, letting you generate API documentation directly from your validation schemas.

> **Building an adapter package?** Most users only need `toJsonSchema()`. If you
> convert Ack schemas to another target format, render from
> `schema.toSchemaModel()` (the canonical `AckSchemaModel`) rather than the JSON
> Schema map — see [Creating Schema Converters](./creating-schema-converter-packages.md).
