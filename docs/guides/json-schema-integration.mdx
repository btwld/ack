...
-   **`additionalProperties`:** `Ack.object(..., additionalProperties: false)` becomes `additionalProperties: false`. Direct `AckSchema.toJsonSchema()` renders `true` as an open object schema (`{}`), while `schema.toSchemaModel().toJsonSchema()` renders it as the boolean `true`; both are valid JSON Schema.
