...
against a dialect by shape. Replacing an older draft's `$schema` string does
not convert its semantics. Tuple-form `items` (a Draft-7 array) is rejected;
Draft-7 ignores `$ref` siblings that this importer applies. Draft-4 boolean
`exclusiveMinimum` and `exclusiveMaximum` values are rejected, and Draft-4
`integer` rejects `1.0` where this importer accepts it. Convert older schemas
to draft 2020-12 and check their validation results before importing them.
...
still import. This can differ from validators such as Ajv, which enables `u`
by default: `pattern: "^.$"` rejects `"😀"` here but accepts it with `u`.
Test patterns in both validators if their results must agree.
...
unevaluated-property checks, conditionals, formats, and an extension
