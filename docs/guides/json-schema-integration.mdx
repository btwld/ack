...
(a map or boolean, not a JSON string) into an `AckSchema<Object, Object>`. The input dialect is draft 2020-12, including when
`$schema` is absent. Other explicitly declared dialects are rejected.
...
Ack schemas; its runtime implementation is internal. It validates JSON values without injecting defaults or applying
coercions. Parsing returns a detached, recursively unmodifiable value.
...
The source document, import report, and executable validator are distinct:
`Ack.fromJsonSchema()` returns the validator; `importJsonSchema()` returns a
`JsonSchemaImportResult` with an abstract `schema`, immutable `diagnostics`,
and `isExact`. Both are strict by default. Only the report-producing API accepts
`allowUnsupported: true`. All entry points delegate to the same compiler.
The importer preserves supported JSON validation semantics,
...
Exports containing unsupported keywords still follow the strictness policy.
...
| Objects | `properties`, `required`, boolean/schema `additionalProperties`, `minProperties`, `maxProperties` |
| Arrays | Single-schema `items`, `minItems`, `maxItems`, `uniqueItems` |
| Strings | `minLength`, `maxLength` (Unicode code points) |
...
keywords include `format`, `pattern`, `multipleOf`, `patternProperties`,
`prefixItems`, `contains`, conditional/dependent/unevaluated keywords, dynamic
references, and custom vocabulary declarations. Unknown extension keywords are
also reported. `format` is not enforced, even for formats Ack's native string
schemas can validate. References to the 2020-12 meta-schema fail strict import.
Explicit partial import reports their omission, and only the remaining
supported assertions constrain the value; the meta-schema itself is not
validated.
...
### Strict imports and partial imports

Strict import is the default. Unsupported features throw
...
the imported validator lossy. Schema resource IDs still need to be unambiguous.

To explicitly accept a partial import:

```dart
final JsonSchemaImportResult imported = importJsonSchema(
  {'type': 'string', 'format': 'email'},
  allowUnsupported: true,
);
assert(!imported.isExact);
for (final issue in imported.diagnostics) {
  print('${issue.documentUri}${issue.pointer}: ${issue.message}');
}
// Enforces string, not email. Export also omits format.
```

Partial import can accept more values than the original. Unsupported assertions
are not retained in exports. To avoid accidentally accepting fewer values, an
incomplete `not` is omitted. An incomplete `oneOf` becomes an inclusive `anyOf`,
preserving its branch shapes but dropping exclusivity, with a diagnostic.
Similarly, `additionalProperties` is omitted alongside unsupported
`patternProperties`, and `items` alongside unsupported `prefixItems`.
...
are errors even in partial mode. The importer checks schema structure and the
keywords it compiles; it is not full meta-schema validation of unused definitions
or unsupported vocabularies. Structural recursion is supported without adding
an arbitrary validation-depth constraint.
...
`importToAck({baseUri, documents, allowUnsupported = false})` returns the report.
Both accept `Map<Uri, jsb.Schema>` bundles. Partial conversion is available only
through `importToAck(allowUnsupported: true)`; inspect its diagnostics before
using its validator.
...
Offline regression fixtures pin the eleven A2UI v0.9 protocol files and basic
catalog to upstream commit `f5e945a93da36a5d126228f2ad6fbe68809ef8f7`.
For this catalog, register the catalog document under
`https://a2ui.org/specification/v0_9/catalog.json`, in addition to the eleven
protocol documents under their retrieval URIs. Its own `$id` is registered
automatically; no source files need to be rewritten.
...
keyword. Strict import reports them. Explicit partial import retains message
and component shapes, but may accept extra component properties. The client
event schema reports its date-time format, and client capabilities report the
three meta-schema references. Test the specific catalog your application uses;
successful validation of examples does not prove complete catalog conformance.

The committed conformance tests contain 600 upstream JSON Schema instance cases
across 178 groups in the supported subset, including supplied remote documents.
