...
2.  **`TextFormField.validator`:** Inside the `validator` function, call `schema.safeParse(value)`. If `result.isFail`, return `result.getError().message` to display the error (see [Error Handling](../core-concepts/error-handling.mdx)).
...
Instead of validating field by field, you can validate a complete data structure (like a map representing the whole form) on submission using [`Ack.object`](../core-concepts/schemas.mdx#object).
...
1.  **Object Schema:** Create an [`Ack.object`](../core-concepts/schemas.mdx#object) schema representing the entire form's data structure.
2.  **Reuse Schemas:** Reuse the individual field schemas within the object schema.
3.  **Cross-Field Validation:** Use object-level [`.refine()`](./custom-validation.mdx#cross-field-rules-with-refine) for rules that depend on multiple fields.
