...
class <Target>SchemaConverter {
  ...
  static Map<String, Object?> _convertSchema(AckSchemaModel schema) {
    return switch (schema) {
      ...
      // Imported fragments keep original JSON Schema keywords. Map targets
      // should emit the generic Draft-7 document; typed builder APIs that
      // cannot represent an untyped keyword bag should throw instead.
      AckImportedSchemaModel() => schema.toJsonSchema(),
    };
  }
