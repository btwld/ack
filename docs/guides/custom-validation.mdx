...
print(priceSchema.validate(-5.0).getError().toString()); // Number must be positive
...
print(result.getError().toString()); // Passwords do not match
```
...
print(schema.validate(-10).getError().toString()); // Price must be greater than zero.
```
...
final userSchema = Ack.object({
  'age': Ack.integer().constrain(IsPositiveConstraint()),
  // ... other fields
});
```
