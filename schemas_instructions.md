...
All schemas expose `safeParse`, `parse`, `parseAs`, and `safeParseAs`. Fluent helpers like `.nullable()`, `.optional()`, `.withDefault()`, `.describe()`, `.refine()`, and `.transform()` are available on every schema unless noted.
...
enabledSchema.parse(true);              // returns true
